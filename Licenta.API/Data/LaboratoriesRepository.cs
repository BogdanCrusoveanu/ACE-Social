...
namespace Licenta.API.Data
{
    public class LaboratoriesRepository : ILaboratoriesRepository
    {
        ...
        public async Task<List<Laboratory>> GetLaboratoriesForUser(int userId)
        {
            ...
            {
                var laboratories = await _context.Laboratories.Where(a => a.TeacherId == userId).ToListAsync();
                return laboratories;
            }
            else
            {
                ...
                return await _context.Laboratories.Where(l => l.SubGroupId == subGroupId).ToListAsync();
            }
        }
